...
export const AssistantBubbleMessages: Record<string, () => string> = {
  ...
  [AssistantEvent.Achievement]: () => pick(["哇！解锁了新成就！", "恭喜获得成就！继续加油~"]),
  [AssistantEvent.TimerReset]: () =>
    pick(["重新开始也没关系~", "调整一下再出发！", "没关系，重新来过~"]),
  tap: () => pick(["嗨~有什么事吗？", "别戳我啦~", "在呢在呢~", "专心工作啦！", "今天也要加油哦~"]),
  headpat: () => pick(["呜…别摸头啦！", "好舒服…再摸一下~", "头发会乱的！", "嘿嘿~"]),
  cheer: () => pick(["你是最棒的！加油！", "冲冲冲！我相信你！", "今天也要元气满满！"]),
  celebrate: () => pick(["撒花~ 🎉", "太棒啦！庆祝一下！", "耶！开心！🎊"]),
};
