...
export function buildAchievements(
  ...
  return configs.map((config) => {
    ...
    const current = unlocked ? config.target : Math.min(rawCurrent, config.target);
...
export function buildAchievementSummary(achievements: AchievementItem[]): AchievementSummaryModel {
  ...
  const achievementPoints = unlockedAchievements.reduce((sum, item) => sum + item.points, 0);
  ...
  const nearestAchievement = lockedAchievements
    .slice()
    .sort((a, b) => b.current / Math.max(b.target, 1) - a.current / Math.max(a.target, 1))[0];
